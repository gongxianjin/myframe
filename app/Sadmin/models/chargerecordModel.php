<?php
/**
 * Created by PhpStorm.
 * User: sun
 * Date: 2016/11/8
 * Time: 15:13
 */

namespace Sadmin\models;

use core\lib\model;

class chargerecordModel extends model
{

    protected $table = 'chargerecord';

    protected $pk = 'id';

    protected $fields = array('id','msgid','type','phone','point','userid','usernickname','createtime','activitykey','activityname','laststate','laststatedesc','lastchargetime','giftrecordid','giftname','ipaddr','restime','flag');

    protected $_valid = array(
        array('id',1,'id不能为空','require'),
        array('createtime',1,'生成时间不能为空','require'),
//        array('msgid',1,'msgid必须是整型值','number'),
//        array('type',0,'type只能是1或2或3','in','1,2,3'),
    );

    protected $_auto = array(
        array('createtime','function','time'),
    );

    //查询一条
    public function getOne($id)
    {
        return $this->db->get($this->table,'*',array($this->pk => $id));
    }

    //查询任意一条
    public function getRow()
    {
        return $this->db->get($this->table,'*');
    }

    //根据key查询一条
    public function getKey($parm)
    {
        return $this->db->get($this->table,'*',$parm);
    }

    //根据Parm查询多条
    public function getlists($parm)
    {
        return $this->db->select($this->table,'*',$parm);
    }

    public function getAll(){
        $ret = $this->db->select($this->table,'*');
        return $ret;
    }

    //添加数据
    public function add($data)
    {
        return $this->db->insert($this->table,$data);
    }

    //删除一条
    public function delOne($id)
    {
        return $this->delete($this->table,array('id'=>$id));
    }

    //修改一条
    public function setOne($id,$data)
    {
        return $this->db->update($this->table,$data,array('id'=>$id));
    }

    public function getCount($data = ''){
        return $this->db->count($this->table,$data);
    }

    public function getcolumnSum($column,$data = ''){
        return $this->db->sum($this->table,$column,$data);
    }

}